...
// ShaderBackground — adapted from the 21st.dev Shader Builder ("Waves").
// Zero dependencies: one WebGL canvas that fills its parent. Drop it behind
// your content with <div className="relative"><ShaderBackground className="absolute inset-0" />…
//
// Two presets share the same mount/resize/visibility/reduced-motion plumbing
// below, selected via the `variant` prop:
//   - "waves"    (default) — the original dark CTA-card background.
//   - "halftone" — dot-field preset used behind the About page differentiators.
...
// "Halftone" preset — dot-field shader used behind the About page
// differentiators. Same uniform schema/API as "Waves" above (u_scene,
// u_shape, u_surface, u_finish, u_transform, u_space, u_cursor); only the
// shade() function differs (dot grid mask instead of a wavy palette band).
const FRAG_HALFTONE = `#ifdef GL_FRAGMENT_PRECISION_HIGH
precision highp float;
#else
precision mediump float;
#endif
...
// Monochrome grayscale ramp (dark → light), converted from the original
// colored palette. Only the hue changed — every other animation parameter
// below is untouched.
const UNIFORMS = {
  colors: [[0.06, 0.06, 0.06], [0.35, 0.35, 0.35], [0.65, 0.65, 0.65], [0.85, 0.85, 0.85], [0.85, 0.85, 0.85], [0.85, 0.85, 0.85], [0.85, 0.85, 0.85], [0.85, 0.85, 0.85]] as [number, number, number][],
  colorCount: 4,
  scale: 1.320,
  intensity: 0.490,
  paramA: 0.840,
...
// Light grayscale pair for the "Halftone" preset — hand-authored normalized
// floats (not read from CSS at runtime; same authoring pattern as UNIFORMS
// above), targeting rgb(240,240,240) → rgb(200,200,200) so the shader stays
// light enough for the About page's existing dark-on-light card text to stay
// legible on top. Remaining 6 slots repeat colors[1] (unused, colorCount: 2).
const UNIFORMS_HALFTONE = {
  colors: [[0.941, 0.941, 0.941], [0.784, 0.784, 0.784], [0.784, 0.784, 0.784], [0.784, 0.784, 0.784], [0.784, 0.784, 0.784], [0.784, 0.784, 0.784], [0.784, 0.784, 0.784], [0.784, 0.784, 0.784]] as [number, number, number][],
  colorCount: 2,
  scale: 1.78,
  intensity: 0.19,
  paramA: 1.00,
...
export function ShaderBackground({
  ...
  className?: string
  variant?: "waves" | "halftone"
}) {
  const canvasRef = useRef<HTMLCanvasElement>(null)

  useEffect(() => {
    const fragSrc = variant === "halftone" ? FRAG_HALFTONE : FRAG
    const uniforms = variant === "halftone" ? UNIFORMS_HALFTONE : UNIFORMS
    ...
    // prefers-reduced-motion: freeze on a single static frame instead of
    // forcing motion. Re-evaluated live if the OS setting changes.
    const reducedMotionQuery = window.matchMedia(
      "(prefers-reduced-motion: reduce)",
    )
    let reducedMotion = reducedMotionQuery.matches
    const baseTimeAnimated = Math.abs(uniforms.timeScale) > 0.0001
    ...
    function render(now: number) {
      ...
      if ((baseTimeAnimated && !reducedMotion) || pointerSettling)
        requestRender()
      else lastNow = null
    }
    requestRender()
